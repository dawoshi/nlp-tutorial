...
namespace base {
    ...
    void FullTokenizer::convert_tokens_to_ids(const std::vector<std::string> &tokens, std::vector<uint64_t> &ids) {
        for (int i = 0; i < tokens.size(); ++i) {
            ids.push_back(convert_token_to_id(tokens[i]));
...
// trim from both ends (in place)
    ...
    void load_vocab(const char *vocab_file, std::unordered_map<std::string, uint64_t> *vocab) {
        ...
        if (!file.is_open()) {
            LOG(ERROR) << "Unable to open vocab file";
	    return;
        }
