...
class BertLabeling(pl.LightningModule):
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--best_checkpoint", type=str, default="output", )
        return parser
...
def main():
    ...
    path_to_best_checkpoint = args.best_checkpoint
    ...
    export_model_path = "mrc_ner.onnx"
    inputs = {
    ...
    start_logits, end_logits, match_logits = model(
                              inputs['input_ids'],
    ...
    torch.onnx.export(model, # model being run
                  (inputs['input_ids'],
                   ...
                   output_names=['start_logits', "end_logits", "span_logits"],   # the model's output names
                   dynamic_axes={'input_ids': symbolic_names,
                                 'attention_mask' : symbolic_names,
                                 'token_type_ids' : symbolic_names,
                                 'start_logits' : symbolic_names,
                                 'end_logits': symbolic_names,
                                 'span_logits': {0:'batch_size',1:'max_seq_len',2:'max_seq_len'}})   # variable length axes/dynamic input
